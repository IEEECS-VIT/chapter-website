  ...
  return (
    <div
      className={`flex-shrink-0 w-[260px] sm:w-[280px] ${
        first ? "ml-6 mr-3" : "mx-3"
      } last:mr-6`}
    >
      <div className="relative bg-gradient-to-br from-[#F8F4ED] to-[#F1ECE5] p-4 sm:p-6 rounded-2xl shadow-lg border border-gray-200/30 flex flex-col items-center justify-center h-full">
        <div className="relative w-[240px] sm:w-[250px] h-[260px] sm:h-[290px] overflow-hidden rounded-xl shadow-inner flex items-center justify-center">
          <img
          ...
          />
          {hasOverlay && (
            <div className="absolute inset-0 bg-gradient-to-t from-[#2C2A2A]/90 via-[#5D5A5A]/70 to-transparent flex items-center justify-center p-6 opacity-0 hover:opacity-100 transition-all duration-500 ease-out backdrop-blur-sm">
              <p className="text-white text-sm sm:text-base text-center font-serif tracking-wide leading-relaxed px-4 sm:px-6 py-4 break-words">
                {overlayText}
              </p>
            </div>
          )}
        </div>
        <h3 className="text-gray-800 text-lg sm:text-[1.3rem] text-center tracking-wider mt-4 sm:mt-6 font-mono hover:text-gray-900 transition-colors duration-300 drop-shadow-sm">
          {title}
        </h3>
      </div>
    </div>
  ...
  useEffect(() => {
    ...
    gsap.to(scroller, {
      ...
      scrollTrigger: {
        trigger: pin,
        start: "top top",
        end: `+=${totalScroll}`, // matches scroll to card width
        scrub: true,
        pin: true,
        anticipatePin: 1,
      },
